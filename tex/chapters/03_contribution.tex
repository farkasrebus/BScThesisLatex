...
The reachability-checking algorithm described in part \ref{sec:tareach} is an obvious choice for the model checking phase, however, it is important to note that the algorithm does not handle discerete variables. The discrete valuation can be encoded into the location (and calculated on the fly) but in this case termination is not ensured (as part \todo{ref} explains).
...
\subsubsection{Graph representation} % ezt lehet jobb lenne nem subsubsecionként?
...
Because of this the initial abstraction can be constructed by assigninga node $\langle l, z_{\emptyset} \rangle$ to each location $l \in L$. The graph can then be compled with edges: for each $e=(l,g,r,l') \in E$ a new edge of the zone graph should be included pointing from $\langle l, z_{\emptyset} \rangle$ to $\langle l', z_{\emptyset} \rangle$.
...
\section{Result}
\todo{Kis szöveg meg sok sok ábra az elkészült algoritmusokról, a ko9mbinálhatóságról, valamint a keretrendszer kiterjesztési lehetőségeiről}
