...
\begin{itemize}
	\item $v$ satisfies $g$ 
	\item $v'$ assigns 0 to any $c \in r$ and assigns $v(c)$ otherwise, and
...
\begin{figure} [b]
	...
	%		\caption{Timed automaton}
		...
		\end{minipage}
	\caption{Timed automaton and its zone graph}
\end{figure}  
...
Since there is no state in $\langle loop, x=y \wedge x \leq 10 \rangle$ where $y \geq 20$ holds, the last transition will never be enabled and the zone graph is finished.
 The result can be seen in Figure \ref{fig:loopfinitegraph}.
...
Unfortunately, it ispossible that the graph described by the previous algorithm becomes infinite. Consider for example the automaton from \cite{bengtsson2004timed} in Figure \ref{fig:loopinfinite}. The only difference between this automaton and the one in figure \ref{fig:loopfinite}, is that in this automaton the transition represented by the loop edge only resets $x$, but not $y$.
