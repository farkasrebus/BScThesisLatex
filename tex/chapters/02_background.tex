...
The (boolean) satisfiability problem (SAT, for short) can be defined as follows \todo{cite Ákos 7}.
\begin{description}
...
SAT is NP-complete \todo{cite Ákos 3} but in practice there are efficient solvers \todo{cite Ákos 4}.
...
The satisfiability problem can be extended to first order logic formulae, but it is undecidable \todo{cite Ákos 11,12}. However, there is a variant of the problem that is applicable, and solvable for most practical problems. The key idea is to formalize structures.

\begin{dfn}
	A \emph{first order theory} $\mathcal{T}$ is a pair $(\Sigma,\mathcal{A})$  where \todo{cite Ákos 2}
	\begin{itemize}
...
\end{dfn}
 ...
 \todo{state, state space definíció}
...
Model checking can be performed by state space exploration for example, however, there are countless other ways.
